...
def synthesize_for_api(name: str) -> None:
...
if __name__ == '__main__':
  ...
  with open('api_test_errors_dict.bin', 'rb') as f:
    result = pickle.load(f)
    
    print('hello')
  
  '''
  api_tests_dict: Dict[str, List[str]] = pickle.load(open('api_test_dict.bin', 'rb'))
  api_freq_dict: Dict[str, int] = pickle.load(open('../crawler/api_freq_dict.bin', 'rb'))
  
  total = 0
  success = 0
  for k, v in api_freq_dict.items():
    total += v
    if len(api_tests_dict[k]) > 0:
      success += v
      
  print(success / total)
  
  # failure_case_analysis('api_test_dict.bin')
  # output_type_analysis('api_test_dict.bin')
  package_success_rate_analysis(api_tests_dict, api_freq_dict)

  # apis = load_apis('torch')
  # api = list(filter(lambda x: x.id == 'torch.zeros_like', apis))[0]
  # collect_valid_tests(api)
