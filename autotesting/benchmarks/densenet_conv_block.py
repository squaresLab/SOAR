...
class TFConv(Model):
    def __init__(self, **kwargs):
        ...
        # inputs should be 224x224x3 images
        '''[SOURCE STRUCTURE CODE STARTS HERE]'''
        self.bn_1 = tf.keras.layers.BatchNormalization(axis=3, epsilon=1.001e-5)
        self.activation_1 = tf.keras.layers.Activation('relu')
        self.conv_1 = tf.keras.layers.Conv2D(128, 1, use_bias=False)
        self.bn_2 = tf.keras.layers.BatchNormalization(axis=3, epsilon=1.001e-5)
        self.activation_2 = tf.keras.layers.Activation('relu')
        self.conv_2 = tf.keras.layers.Conv2D(32, 3, padding='same', use_bias=False)
