...
class TFConv(Model):
    def __init__(self, **kwargs):
        ...
        self.in = tf.keras.layers.Embedding(50, 10)
        self.emb = tf.keras.layers.Conv1D(filters=64, kernel_size=3, strides=1)
        self.act = tf.keras.layers.ReLU()
        self.gru1 = tf.keras.layers.Conv1D(filters=64, kernel_size=3, strides=1)
        self.act = tf.keras.layers.ReLU()
        self.gru2 = tf.keras.layers.MaxPool1D(pool_size=2)
        self.flat = tf.keras.layers.Flatten()
        self.fcc = tf.keras.layers.Dense(100)
        self.act2 = tf.keras.layers.ReLU()
        self.fcc = tf.keras.layers.Dense(10)
        self.act2 = tf.keras.layers.Softmax()
