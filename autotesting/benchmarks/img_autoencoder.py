...
class TFConv(Model):
    def __init__(self, **kwargs):
        ...
        self.x2 = tf.keras.layers.Conv2D(filters=32, kernel_size=3, strides=(2, 2))
        self.xx2 = tf.keras.layers.ReLU()
        self.x3 = tf.keras.layers.Conv2D(filters=64, kernel_size=3, strides=(2, 2))
        self.xx3 = tf.keras.layers.ReLU()
        self.x4 = tf.keras.layers.Flatten()
        self.x5 = tf.keras.layers.Dense(10)
        self.x7 = tf.keras.layers.Dense(units=1568)
        self.xx4 = tf.keras.layers.ReLU()
        self.x8 = tf.keras.layers.Reshape(target_shape=(7, 7, 32))
        self.x9 = tf.keras.layers.Conv2DTranspose(filters=64, kernel_size=3, strides=2)
        self.xx5 = tf.keras.layers.ReLU()
        self.x10 = tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=3, strides=2)
        self.xx6 = tf.keras.layers.ReLU()
        self.x11 = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=3, strides=1)
        self.xx7 = tf.keras.layers.ReLU()
