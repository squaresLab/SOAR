...
class TFConv(Model):
    def __init__(self, **kwargs):
        ...
        self.conv = tf.keras.layers.Conv2D(kernel_size=5, strides=1, filters=16)
        self.act = tf.keras.layers.ReLU()
        self.max = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)
        self.conv2 = tf.keras.layers.Conv2D(kernel_size=5, strides=1, filters=36)
        self.act2 = tf.keras.layers.ReLU()
        self.max2 = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)
        self.flatten = tf.keras.layers.Flatten()
        self.fully = tf.keras.layers.Dense(128)
